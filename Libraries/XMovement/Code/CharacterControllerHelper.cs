...
public struct CharacterControllerHelper
{
	...
	public bool TreatSteepLowSlopesAsWalkable;
	public float SteepSlopeProbeDistance;
	public float SteepSlopeRiseLimit;
	public SceneTrace Trace;
	public bool TouchedSteepSlope { get; private set; }
	...
	public CharacterControllerHelper( SceneTrace trace, Vector3 position, Vector3 velocity ) : this()
	{
		...
		SteepSlopeProbeDistance = 12.0f;
		SteepSlopeRiseLimit = float.PositiveInfinity;
		Trace = trace;
	}
	...
	public float TryMoveWithStep( float timeDelta, float stepsize )
	{
		...
		if ( TreatSteepLowSlopesAsWalkable && stepsize > 0.001f && SteepSlopeRiseLimit > 0.001f )
		{
			var directTrace = TraceFromTo( Position, Position + Velocity * timeDelta );
			var horizontalVelocity = Velocity.WithZ( 0 );
			var slopeTrace = directTrace;

			if ( !horizontalVelocity.IsNearZeroLength )
			{
				var probeDistance = Math.Clamp( SteepSlopeProbeDistance, 0.5f, stepsize );
				var probeTrace = TraceFromTo( Position, Position + horizontalVelocity.Normal * probeDistance );
				var probeAngle = probeTrace.Normal.Angle( Vector3.Up );

				if ( probeTrace.Hit && probeAngle > MaxStandableAngle && probeAngle <= 90.0f )
					slopeTrace = probeTrace;
			}

			if ( !directTrace.Hit && !slopeTrace.Hit )
			{
				Position = directTrace.EndPosition;
				return directTrace.Fraction;
			}

			var directHitAngle = slopeTrace.Normal.Angle( Vector3.Up );
			if ( directHitAngle > MaxStandableAngle
				&& directHitAngle <= 90.0f
				&& slopeTrace.Normal.z > 0.001f )
			{
				var horizontalNormal = slopeTrace.Normal.WithZ( 0 );
				var inwardSpeed = -Velocity.WithZ( 0 ).Dot( horizontalNormal.Normal );

				if ( inwardSpeed > 0.001f )
				{
					var originalVerticalVelocity = Velocity.z;
					var redirectedVelocity = Velocity.WithZ( inwardSpeed * horizontalNormal.Length / slopeTrace.Normal.z );
					var rise = redirectedVelocity.z * timeDelta;

					if ( rise > SteepSlopeRiseLimit )
						redirectedVelocity *= SteepSlopeRiseLimit / rise;

					Velocity = redirectedVelocity;
					var redirectedFraction = TryMove( timeDelta );
					var gainedHeight = Position.z - startPosition.z;

					if ( gainedHeight > 0.001f )
					{
						var settleTrace = TraceFromTo( Position, Position + Vector3.Down * (gainedHeight + 0.03125f) );
						if ( settleTrace.Hit && !settleTrace.StartedSolid )
							Position = settleTrace.EndPosition;
					}

					Velocity = Velocity.WithZ( originalVerticalVelocity );
					TouchedSteepSlope = true;
					return redirectedFraction;
				}
			}
		}
		...
		stepMove.TraceMove( Vector3.Up * (stepsize + 0.03125f) );
		...
		// Move across using the bounded probe delta.
		var stepFraction = stepMove.TraceMove( moveDelta );

		// Move back down
		var tr = stepMove.TraceMove( Vector3.Down * (stepsize + 0.03125f) );

		// if we didn't land on something, return
		if ( !tr.Hit ) return fraction;

		var landingAngle = tr.Normal.Angle( Vector3.Up );
		var landingHeight = stepMove.Position.z - startPosition.z;

		if ( landingAngle > MaxStandableAngle )
			return fraction;

		if ( landingHeight > stepsize + 0.03125f )
			return fraction;
		...
		Velocity = stepMove.Velocity.WithZ( Velocity.z );
